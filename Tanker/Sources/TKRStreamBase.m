...
- (nullable instancetype)init
{
  if (self = [super init])
  {
    ...
    CFDictionaryKeyCallBacks runLoopsModesKeyCallBacks = {0, NULL, NULL, NULL, CFEqual, CFHash};
    CFDictionaryValueCallBacks runLoopsModesValueCallBacks = {0, CFRetainCallBack, CFReleaseCallBack, NULL, CFEqual};
...
- (BOOL)isOpen
{
  return (status != NSStreamStatusNotOpen && status != NSStreamStatusOpening && status != NSStreamStatusClosed &&
...
- (void)stream:(NSStream*)aStream handleEvent:(NSStreamEvent)eventCode
{
  if (aStream == self)
  {
    ...
    case NSStreamEventOpenCompleted:
      ...
      {
        self->clientCallback((__bridge CFReadStreamRef)self, kCFStreamEventOpenCompleted, self->clientContext.info);
      }
      break;
    ...
    case NSStreamEventHasBytesAvailable:
      ...
      {
        self->clientCallback((__bridge CFReadStreamRef)self, kCFStreamEventHasBytesAvailable, self->clientContext.info);
      }
      break;
    ...
    case NSStreamEventErrorOccurred:
      ...
      {
        self->clientCallback((__bridge CFReadStreamRef)self, kCFStreamEventErrorOccurred, self->clientContext.info);
      }
      break;
    ...
    case NSStreamEventEndEncountered:
      ...
      {
        self->clientCallback((__bridge CFReadStreamRef)self, kCFStreamEventEndEncountered, self->clientContext.info);
      }
      break;
...
- (void)addMode:(CFStringRef)mode forRunLoop:(CFRunLoopRef)runLoop
{
  ...
  {
    CFSetCallBacks modesSetCallBacks = {0, CFRetainCallBack, CFReleaseCallBack, NULL, CFEqual, CFHash};
